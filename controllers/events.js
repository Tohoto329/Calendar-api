...
const addEvents = async (req, res) => {
    let { eventDate, eventName, eventPlace, priority, startTime, endTime } = req.body;
    let { email } = req.user;
...
const allEvents = async(req,res) => {
    ...
    try{
        ...
        const [events, totalEvents] = await Promise.all([
            EventModel.find({email}).sort().skip(skip).limit(limit),
        ...
        logger.error(`Error in get all events: ${err.message}`);
        res.status(500).json({error:'Internal Server Error'});
    }
}
